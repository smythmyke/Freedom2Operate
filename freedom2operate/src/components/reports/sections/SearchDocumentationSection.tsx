...
export const SearchDocumentationSection: React.FC<SearchDocumentationSectionProps> = ({
  ...
  const handleAddDatabase = (input: string) => {
    if (!input.trim()) return;
    const [name, dates] = input.trim().split('|');
    if (!name || !dates) return;

    const [startDate, endDate] = dates.split('-').map(d => new Date(d.trim()));
    ...
    onUpdate({
      databases: [...documentation.databases, newDatabase],
    });
    setNewDatabase('');
  };
        ...
        <Grid item xs={12}>
          ...
          <Box display="flex" gap={1} flexWrap="wrap" mb={1}>
            {documentation.databases.map((database, index) => (
              <Chip
                key={index}
                label={`${database.name} (${database.coverageDates.start.toLocaleDateString()} - ${database.coverageDates.end.toLocaleDateString()})`}
          ...
          {!disabled && (
            <Box display="flex" gap={1}>
              <TextField
                ...
                placeholder="Name | YYYY-MM-DD - YYYY-MM-DD"
              />
              <Button
                variant="contained"
                size="small"
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={() => {
              ...
              handleSaveQuery({
                iteration: queryDialog.index === -1 ? documentation.searchQueries.length + 1 : queryDialog.editingQuery?.iteration || 1,
                database,
                query,
                resultCount: parseInt(resultCount, 10) || 0,
                date: new Date(date),
                refinementReason: queryDialog.editingQuery?.refinementReason
              });
            }}
          >
            Save
