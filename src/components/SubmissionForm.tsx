    ...
    const loadDraft = async () => {
      ...
      try {
        ...
        if (!draftDoc.empty) {
          ...
          const consultationDateTime = draftData.consultationDateTime || '';
          const [date = '', time = ''] = consultationDateTime.split('T');
  ...
  const handlePaymentSuccess = async (paymentDetails: PayPalPaymentDetails) => {
    try {
      ...
      const submissionData = {
        ...
        paymentDetails,
        userId: currentUser?.uid || '',
        consultationDateTime: formData.requestConsultation && formData.consultationDate && formData.consultationTime ? 
          `${formData.consultationDate}T${formData.consultationTime}` : '',
      };
  ...
  const handleSubmitForReview = async () => {
    try {
      ...
      await addDoc(collection(db, 'progress'), {
        ...
        currentStep: 0,
        status: 'Pending Review',
        notes: 'Submission received and pending admin review.',
        createdAt: new Date(),
      });
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            helperText="Select a date at least 24 hours in advance"
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            helperText="Select your preferred time (30-min intervals)"
                            required={formData.requestConsultation}
                          />
                        </Grid>
                      </Grid>
            ...
            <Grid item xs={12}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Describe the key technical aspects that make your invention unique. Each feature should be a specific technical element, not a benefit or result. Long features may be separated into multiple features, or multiple features may be combined into one.
                  </Typography>
                </Box>
                <FormControlLabel
                  control={
                ...
                {formData.requestConsultation && (
                  <>
                    ...
                    <Typography sx={{ color: 'primary.main', ml: 2 }}>
                      Preferred Time: {new Date(formData.consultationDate + 'T' + formData.consultationTime).toLocaleString()}
                    </Typography>
                  </>
                )}
              </Grid>
